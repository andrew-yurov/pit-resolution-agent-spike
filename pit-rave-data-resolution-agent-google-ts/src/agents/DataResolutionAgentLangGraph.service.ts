// import { ChatOpenAI, AzureOpenAI } from '@langchain/openai';
...
// import type { ResolutionResult, AgentMessage, Role, Trace } from './IDataResolutionAgentService';
// import logger from '../logger';

// // Define the output schema using Zod
// const DataResolutionOutputSchema = z.object({
// 	selectedSources: z.array(z.string()).describe('Array of module_key strings for the selected data sources'),
// 	reasoning: z.string().describe('Detailed explanation of why these sources were selected')
// });
...
// export type ModelProvider = 'openai' | 'anthropic' | 'azureOpenAi';
// type BaseChatModel = ChatOpenAI | ChatAnthropic | AzureOpenAI;
...
// 		const hasSystemMessage = messages.some((msg: any) => msg._getType() === 'system');

// 		const fullMessages = hasSystemMessage ? messages : [new SystemMessage(systemInstruction), ...messages];

// 		logger.info(`[LangGraph Agent] Calling model with ${fullMessages.length} messages`);
...
// 			logger.info(`[LangGraph Parse] Extracted ${selectedSources.length} sources`);
// 		} catch (error: any) {
// 			logger.error(`[LangGraph Parse] Failed to parse output: ${error.message}`);
...
// 			logger.info(`[LangGraph] Resolution complete: ${selectedSources.length} sources selected`);
...
// 		try {
// 			logger.info(`[LangGraph Tools] Starting two-interaction resolution for: "${prompt}"`);
...
// 			workflow.addNode('agent', async (state: typeof MessagesAnnotation.State) => {
// 				const messages = state.messages;
// 				const hasSystemMessage = messages.some((msg: any) => msg._getType() === 'system');
// 				const fullMessages = hasSystemMessage ? messages : [new SystemMessage(toolSystemMessage), ...messages];

// 				logger.info(`[LangGraph Tools] Agent node: calling model with ${fullMessages.length} messages`);
...
// 			workflow.addNode('select_sources', async (state: typeof MessagesAnnotation.State) => {
// 				const messages = state.messages;

// 				logger.info(`[LangGraph Tools] Selection node: analyzing tool results`);
...
// 				const response = await selectionModel.invoke([selectionPrompt, ...messages]);
...
// 				logger.info(`[LangGraph Tools] Routing decision: ${hasToolCalls ? 'tools' : 'select_sources'}`);
...
// 			logger.info(`[LangGraph Tools] Resolution complete: ${selectedSources.length} sources selected`);
